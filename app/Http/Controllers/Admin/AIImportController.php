...
class AIImportController extends Controller
{
    ...
    {
        return inertia('Admin/Questions/AIImport', [
            'exam' => $exam,
        ]);
    }
    ...
    public function generate(Request $request, Exam $exam)
    {
        ...
        if (!$apiKey) {
            // Return the same page with manual editor fallback
            return inertia('Admin/Questions/AIImport', [
                'exam' => $exam,
                'preview' => [],
                'form' => $data,
        ...
        // Use OpenAI-compatible HTTP API
        ...
        if (str_starts_with($content, '```')) {
            // Remove typical code fences
            $content = preg_replace('/^```[a-zA-Z]*\n|```$/m', '', $content);
        }
        // Try to extract the first JSON array substring if extra text exists
        ...
        // Return preview to page to allow review/pagination and confirmation
        return inertia('Admin/Questions/AIImport', [
            'exam' => $exam,
            'preview' => $items,
            'form' => $data,
    ...
    public function confirm(Request $request, Exam $exam)
    {
        ...
        return redirect()->route('admin.exams.show', $exam->id)->with('success', "AI mengimpor {$created} soal.");
    }
}
