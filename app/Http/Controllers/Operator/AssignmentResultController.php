...
class AssignmentResultController extends Controller
{
    public function index(Assignment $assignment, Request $request)
    {
        $subs = AssignmentSubmission::with('student')
            ...
            ->paginate(20)
            ->withQueryString();
        return inertia('Operator/Assignments/Results/Index',[ 'assignment'=>$assignment, 'submissions'=>$subs, 'filters'=>['q'=>$request->q] ]);
    }
    public function show(Assignment $assignment, AssignmentSubmission $submission)
    {
        ...
        return inertia('Operator/Assignments/Results/Show',[ 'assignment'=>$assignment, 'submission'=>$submission, 'answers'=>$answers ]);
    }
