...
class TryoutResultController extends Controller
{
    public function index(Tryout $tryout, Request $request)
    {
        $attempts = TryoutAttempt::with('student')
            ...
            ->paginate(20)
            ->withQueryString();
        return inertia('Teacher/Tryouts/Results/Index',[ 'tryout'=>$tryout, 'attempts'=>$attempts, 'filters'=>['q'=>$request->q] ]);
    }
    public function show(Tryout $tryout, TryoutAttempt $attempt)
    {
        ...
        return inertia('Teacher/Tryouts/Results/Show',[ 'tryout'=>$tryout, 'attempt'=>$attempt, 'answers'=>$answers ]);
    }
