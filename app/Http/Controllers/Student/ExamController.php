...
class ExamController extends Controller
{
    ...
    public function startExam($id)
    {
        ...
        // block resume if marked exited
        if ($grade && $grade->status === 'exited') {
            return redirect()->route('student.exams.confirmation', $id)
                ->with('error', 'Sesi ujian Anda sebelumnya terdeteksi keluar. Hubungi operator untuk membuka kembali ujian.');
        }
    ...
    public function show($id, $page)
    {
        ...
        // prevent access if exited
        $gradeCheck = Grade::where('exam_id', $exam_group->exam->id)
                    ->where('exam_session_id', $exam_group->exam_session->id)
                    ->where('student_id', auth()->guard('student')->user()->id)
                    ->first();
        if ($gradeCheck && $gradeCheck->status === 'exited') {
            return redirect()->route('student.exams.confirmation', $id)
                ->with('error', 'Sesi ujian Anda sebelumnya terdeteksi keluar. Hubungi operator untuk membuka kembali ujian.');
        }
    ...
    $question_active = Answer::with('question.exam')
        ...
        //update monitoring info (current question + last activity)
        if ($exam_group) {
            $gradeMon = Grade::where('exam_id', $exam_group->exam->id)
                        ->where('exam_session_id', $exam_group->exam_session->id)
                        ->where('student_id', auth()->guard('student')->user()->id)
                        ->first();
            if ($gradeMon) {
                $gradeMon->current_question = (int) $page;
                $gradeMon->last_activity_at = now();
                $gradeMon->save();
            }
        }
    ...
    $extraMs = 0;
        if ($grade) {
            if ($request->filled('duration')) {
                // don't reduce DB duration below current (to support admin add time)
                $incoming = (int) $request->duration;
                if ($grade->duration > $incoming) {
                    $extraMs = $grade->duration - $incoming;
                } else {
                    $grade->duration = $incoming;
                }
            }
            if ($request->filled('current_question')) {
                $grade->current_question = $request->current_question;
            }
            $grade->last_activity_at = now();
            $grade->save();
        }

        // ignore small drift less than 1 minute
        if ($extraMs < 60000) {
            $extraMs = 0;
        }
        return response()->json(['success' => true, 'duration' => $grade?->duration, 'extra_ms' => $extraMs]);
    }
    ...
    /**
     * exitExam: mark in-progress exams as exited for current student
     */
