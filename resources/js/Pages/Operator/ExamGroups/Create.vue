<template>
  <Head>
    <title>Enrolle Siswa - Operator</title>
  </Head>
  <div class="container-fluid mb-5 mt-5">
    <div class="row">
      <div class="col-md-12">
  <Link :href="`/operator/exam-sessions/${exam_session.id}`" class="btn btn-md btn-primary border-0 shadow mb-3"><i class="fa fa-long-arrow-alt-left me-2"></i> Kembali</Link>
...
export default {
  ...
  setup(props) {
    ...
    const submit = () => {
      router.post(`/operator/exam-sessions/${props.exam_session.id}/enrolle/store`, { exam_id: form.exam_id, student_id: form.student_id }, {
        onSuccess: () => Swal.fire({ title: 'Success!', text: 'Enrolle Siswa Berhasil Disimpan!.', icon: 'success', timer: 2000, showConfirmButton: false }),
      });
    };
    return { form, selectAll, submit };
  }
}
</script>
