<template>
  ...
  <div class="container-fluid mt-4 mb-5">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <div>
        <Link :href="`/operator/assignments`" class="btn btn-light me-2">Kembali</Link>
        <Link :href="`/operator/assignments/${assignment.id}/questions/create`" class="btn btn-primary">Tambah Soal</Link>
      </div>
    </div>
            ...
            <tr v-for="(q,i) in questions.data" :key="q.id">
              ...
              <td>
                <div class="btn-group btn-group-sm">
                  <Link :href="`/operator/assignments/${assignment.id}/questions/${q.id}/edit`" class="btn btn-warning">Edit</Link>
                  <Link :href="`/operator/assignments/${assignment.id}/questions/${q.id}`" method="delete" as="button" class="btn btn-danger" confirm="Hapus soal ini?">Hapus</Link>
                </div>
              </td>
            </tr>
          </tbody>
...
import { ref } from 'vue';
export default { layout: OperatorLayout, components:{Head,Link,Pagination}, props:{ assignment:Object, questions:Object, filters:Object }, setup(props){ const search=ref(props.filters?.q||''); const submitSearch=()=>{ router.get(`/operator/assignments/${props.assignment.id}/questions`, { q: search.value }); }; return { search, submitSearch }; } }
</script>
<style></style>
