<template>
  ...
  <div class="mb-3 d-flex justify-content-between align-items-center">
    <Link :href="`/operator/assignments`" class="btn btn-sm btn-outline-secondary"><i class="fa fa-long-arrow-alt-left me-1"/> Kembali</Link>
  ...
  <div class="card border-0 shadow">
    <div class="card-header d-flex justify-content-between align-items-center">
      ...
      <a :href="`/operator/assignments/${assignment.id}/results-export`" class="btn btn-sm btn-success"><i class="fa fa-download me-1"/> Export CSV</a>
    </div>
      ...
      <table class="table table-striped mb-0">
        ...
        <tbody>
          <tr v-for="(s,i) in submissions.data" :key="s.id">
            ...
            <td>{{ s.score }}</td>
            <td>
              <Link :href="`/operator/assignments/${assignment.id}/results/${s.id}`" class="btn btn-sm btn-info"><i class="fa fa-eye"/></Link>
            </td>
          </tr>
...
import { ref } from 'vue';
export default { layout: OperatorLayout, components:{Head,Link,Pagination}, props:{ assignment:Object, submissions:Object, filters:Object }, setup(props){ const search=ref(props.filters?.q||''); const submitSearch=()=>{ router.get(`/operator/assignments/${props.assignment.id}/results`, { q: search.value }); }; return { search, submitSearch }; } };
</script>
<style></style>
