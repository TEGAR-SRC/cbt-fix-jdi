<template>
  ...
  <div class="mb-3"><Link :href="`/operator/tryouts/${tryout.id}/questions`" class="btn btn-sm btn-outline-secondary"><i class="fa fa-long-arrow-alt-left me-1"/> Kembali</Link></div>
  <div class="row">
    <div class="col-md-4">
...
export default { layout: OperatorLayout, components:{Head,Link}, props:{ tryout:Object, preview:{type:Array,default:()=>[]}, form:Object, error:String }, setup(props){
  const form = useForm({ category: props.form?.category||'', count: props.form?.count||10, difficulty: props.form?.difficulty||'' });
  const submitGenerate=()=>{ form.post(`/operator/tryouts/${props.tryout.id}/questions/ai-import/generate`); };
  const submitConfirm=()=>{ router.post(`/operator/tryouts/${props.tryout.id}/questions/ai-import/confirm`, { items: props.preview }); };
