<template>
  ...
  <div class="mb-3 d-flex justify-content-between align-items-center">
    <div>
      <Link :href="`/operator/tryouts`" class="btn btn-sm btn-primary"><i class="fa fa-long-arrow-alt-left me-1"/> Kembali</Link>
    </div>
  ...
  <div class="card border-0 shadow">
    <div class="card-header d-flex justify-content-between align-items-center">
      ...
      <a :href="`/operator/tryouts/${tryout.id}/results-export`" class="btn btn-sm btn-success"><i class="fa fa-download me-1"/> Export CSV</a>
    </div>
      ...
      <table class="table table-striped mb-0">
        ...
        <tbody>
          <tr v-for="(a,i) in attempts.data" :key="a.id">
            ...
            <td>{{ a.score }}</td>
            <td><Link :href="`/operator/tryouts/${tryout.id}/results/${a.id}`" class="btn btn-sm btn-info"><i class="fa fa-eye"/></Link></td>
          </tr>
...
import { ref } from 'vue';
export default { layout: OperatorLayout, components:{Head,Link,Pagination}, props:{ tryout:Object, attempts:Object, filters:Object }, setup(props){ const query=ref(props.filters?.q||''); const searchSubmit=()=>{ router.get(`/operator/tryouts/${props.tryout.id}/results`, { q: query.value }); }; return { query, searchSubmit }; } };
</script>
<style></style>
