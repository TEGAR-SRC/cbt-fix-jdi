<template>
  <Head><title>Edit Tryout - Operator</title></Head>
  <div class="container-fluid mb-5 mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h5 class="mb-0">Edit Tryout</h5>
      <Link href="/operator/tryouts" class="btn btn-sm btn-outline-secondary">Kembali</Link>
    </div>
          ...
          <div class="row g-3">
            ...
            <div class="col-12">
              <div class="form-check mt-2">
                <input class="form-check-input" type="checkbox" id="publishNowEditO" v-model="publishNow"/>
                <label class="form-check-label" for="publishNowEditO">Publikasikan sekarang</label>
              </div>
            </div>
          </div>
          <div class="mt-4 d-flex gap-2">
            <button class="btn btn-primary" :disabled="form.processing">Simpan</button>
            <Link :href="`/operator/tryouts/${tryout.id}`" method="delete" as="button" class="btn btn-danger" preserve-scroll confirm="Hapus tryout ini?">Hapus</Link>
          </div>
        </form>
      </div>
    </div>
