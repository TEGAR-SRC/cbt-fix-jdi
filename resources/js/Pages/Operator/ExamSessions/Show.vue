<template>
  <Head>
    <title>Detail Sesi Ujian - Operator</title>
  </Head>
  <div class="container-fluid mb-5 mt-5">
    <div class="row">
      <div class="col-md-12">
  <Link href="/operator/exam-sessions" class="btn btn-md btn-primary border-0 shadow mb-3" type="button"><i class="fa fa-long-arrow-alt-left me-2"></i> Kembali</Link>
          ...
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-md-6 text-end">
                <Link :href="`/operator/exam-sessions/${exam_session.id}/enrolle/create`" class="btn btn-md btn-primary border-0 shadow me-2"><i class="fa fa-user-plus"></i> Enrolle Siswa</Link>
              </div>
            </div>
  ...
  setup(props) {
    const destroy = (exam_group_id) => {
      Swal.fire({ title: 'Apakah Anda yakin?', text: 'Data tidak dapat dikembalikan!', icon: 'warning', showCancelButton: true, confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: 'Ya, hapus!' })
        .then((result) => { if (result.isConfirmed) { router.delete(`/operator/exam-sessions/${props.exam_session.id}/enrolle/${exam_group_id}/destroy`); } });
    };
    return { destroy };
  }
}
