<template>
  ...
  <Link :href="`${basePath}/students`" class="btn btn-md btn-primary border-0 shadow mb-3"><i class="fa fa-long-arrow-alt-left me-2"></i> Kembali</Link>
        <div class="card border-0 shadow">
          <div class="card-body">
            <h5><i class="fa fa-user-edit"></i> Edit Siswa</h5>
            <hr>
...
export default {
  ...
  props: { errors: Object, student: Object, classrooms: Array },
  setup(props) {
    const form = reactive({ name: props.student.name, nisn: props.student.nisn, gender: props.student.gender, classroom_id: props.student.classroom_id, password: '', password_confirmation: '' });
    const basePath = `/${(window?.app?.page?.url || window.location.pathname).split('/')[1] || 'teacher'}`;
    const submit = () => { router.put(`${basePath}/students/${props.student.id}`, { ...form }); };
