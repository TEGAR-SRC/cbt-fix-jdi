<template>
  ...
  <div class="mb-3 d-flex justify-content-between align-items-center">
    <Link :href="`/teacher/assignments/${assignment.id}/results`" class="btn btn-sm btn-outline-secondary"><i class="fa fa-long-arrow-alt-left me-1"/> Kembali</Link>
    <div class="btn-group">
...
export default { layout: TeacherLayout, components:{Head,Link}, props:{ assignment:Object, submission:Object, answers:Array }, setup(props){ const forceFinish=()=>{ if(confirm('Yakin paksa selesai?')){ router.post(`/teacher/assignments/${props.assignment.id}/results/${props.submission.id}/force-finish`); } }; const reopen=()=>{ if(confirm('Yakin buka kunci?')){ router.post(`/teacher/assignments/${props.assignment.id}/results/${props.submission.id}/reopen`); } }; return { forceFinish, reopen }; } };
</script>
<style></style>
