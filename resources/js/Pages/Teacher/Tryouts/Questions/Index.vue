<template>
  <Head><title>Detail Tryout - Guru</title></Head>
  <div class="container-fluid mb-5 mt-4">
    <Link :href="'/teacher/tryouts'" class="btn btn-sm btn-outline-secondary mb-3"><i class="fa fa-long-arrow-alt-left me-1"/>Kembali</Link>
    ...
    <div class="card border-0 shadow">
      <div class="card-body">
        ...
        <div class="mb-2">
          <Link :href="`/teacher/tryouts/${tryout.id}/questions/create`" class="btn btn-sm btn-primary me-2"><i class="fa fa-plus-circle"/> Tambah</Link>
          <Link :href="`/teacher/tryouts/${tryout.id}/questions/import`" class="btn btn-sm btn-success me-2"><i class="fa fa-file-import"/> Import</Link>
          <Link :href="`/teacher/tryouts/${tryout.id}/questions/ai-import`" class="btn btn-sm btn-warning text-white"><i class="fa fa-robot"/> AI Import</Link>
        </div>
            ...
            <tbody>
              <tr v-for="(q,index) in questions.data" :key="q.id">
                ...
                <td class="text-center">
                  <Link :href="`/teacher/tryouts/${tryout.id}/questions/${q.id}/edit`" class="btn btn-sm btn-outline-primary me-1"><i class="fa fa-pencil-alt"/></Link>
                  <Link :href="`/teacher/tryouts/${tryout.id}/questions/${q.id}`" method="delete" as="button" class="btn btn-sm btn-outline-danger" confirm="Hapus soal ini?"><i class="fa fa-trash"/></Link>
                </td>
              </tr>
            </tbody>
          </table>
...
import { ref } from 'vue';
export default { layout: TeacherLayout, components:{Head,Link,Pagination}, props:{ tryout:Object, questions:Object, filters:Object }, setup(props){ const search=ref(props.filters?.q||''); const submitSearch=()=>{ router.get(`/teacher/tryouts/${props.tryout.id}/questions`, { q: search.value }); }; return { search, submitSearch }; } }
</script>
