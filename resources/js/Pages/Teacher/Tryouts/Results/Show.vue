<template>
  ...
  <div class="mb-3"><Link :href="`/teacher/tryouts/${tryout.id}/results`" class="btn btn-sm btn-primary"><i class="fa fa-long-arrow-alt-left me-1"/> Kembali</Link></div>
  <div class="card border-0 shadow mb-4">
    ...
    <div class="card-body">
      ...
      <div class="d-flex gap-2">
        <Link v-if="!attempt.finished_at" as="button" method="post" :href="`/teacher/tryouts/${tryout.id}/results/${attempt.id}/force-finish`" class="btn btn-sm btn-danger" onclick="return confirm('Hentikan & selesaikan sekarang?')">Hentikan</Link>
        <Link v-else as="button" method="post" :href="`/teacher/tryouts/${tryout.id}/results/${attempt.id}/reopen`" class="btn btn-sm btn-warning" onclick="return confirm('Buka kembali attempt ini?')">Buka Kembali</Link>
      </div>
    </div>
  </div>
...
export default { layout: TeacherLayout, components:{Head,Link}, props:{ tryout:Object, attempt:Object, answers:Array } };
</script>
<style></style>
