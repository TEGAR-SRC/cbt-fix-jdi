<template>
  ...
  <Link :href="`${basePath}/exams/create`" class="btn btn-primary">Buat Ujian</Link>
      </div>
          ...
          <table class="table table-hover mb-0">
            ...
            <tbody>
              <tr v-for="exam in exams.data" :key="exam.id">
                ...
                <td class="text-end">
                  <Link :href="`${basePath}/exams/${exam.id}`" class="btn btn-sm btn-outline-secondary me-2">Detail</Link>
                  <Link :href="`${basePath}/exams/${exam.id}/edit`" class="btn btn-sm btn-outline-primary me-2">Edit</Link>
                  <Link :href="`${basePath}/exams/${exam.id}`" method="delete" as="button" class="btn btn-sm btn-outline-danger">Hapus</Link>
                </td>
              </tr>
...
export default { 
  ...
  methods:{
    search(){
      ...
      router.get(`${this.basePath}/exams`, params, { preserveState: true, replace: true })
    }
  }
}
</script>
