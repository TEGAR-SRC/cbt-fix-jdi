      ...
      <div class="col-md-8">
        <div class="row">
          <div class="col-md-3 col-12 mb-2">
            <Link :href="`${basePath}/exam-sessions/create`" class="btn btn-md btn-primary border-0 shadow w-100" type="button"><i class="fa fa-plus-circle"></i> Tambah</Link>
          </div>
                ...
                <tbody>
                  <tr v-for="(exam_session, index) in exam_sessions.data" :key="index">
                    ...
                    <td class="text-center">
                      <Link :href="`${basePath}/exam-sessions/${exam_session.id}`" class="btn btn-sm btn-primary border-0 shadow me-2" type="button"><i class="fa fa-plus-circle"></i></Link>
                      <Link :href="`${basePath}/exam-sessions/${exam_session.id}/edit`" class="btn btn-sm btn-info border-0 shadow me-2" type="button"><i class="fa fa-pencil-alt"></i></Link>
...
export default {
  ...
  props: { exam_sessions: Object },
  setup() {
    const search = ref('' || (new URL(document.location)).searchParams.get('q'));
    const basePath = `/${(window?.app?.page?.url || window.location.pathname).split('/')[1] || 'teacher'}`;
    const handleSearch = () => { router.get(`${basePath}/exam-sessions`, { q: search.value }); };
    const destroy = (id) => {
      Swal.fire({
      ...
      }).then((result) => { if (result.isConfirmed) { router.delete(`${basePath}/exam-sessions/${id}`); Swal.fire({ title: 'Deleted!', text: 'Sesi Ujian Berhasil Dihapus!.', icon: 'success', timer: 2000, showConfirmButton: false }); } });
    };
    return { search, handleSearch, destroy, basePath };
  }
}
</script>
