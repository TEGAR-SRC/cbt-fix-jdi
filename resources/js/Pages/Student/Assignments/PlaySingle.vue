<template>
  <Head>
    <title>Tugas No. {{ page }} - {{ assignment.title }}</title>
  </Head>
  <div class="row mb-5">
    <div class="col-md-7">
      <div class="card border-0 shadow">
        ...
        <div class="card-footer">
          <div class="d-flex justify-content-between">
            <div>
              <button v-if="page>1" @click.prevent="goto(page-1)" class="btn btn-gray-400 btn-sm">Sebelumnya</button>
            </div>
            <div>
              <button v-if="page < all_questions.length" @click.prevent="goto(page+1)" class="btn btn-gray-400 btn-sm">Selanjutnya</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="col-md-5">
      <div class="card border-0 shadow">
        ...
        <div class="card-footer">
          <button @click="finishConfirm" class="btn btn-danger w-100">Selesai Tugas</button>
        </div>
      </div>
    </div>
  </div>
...
export default { layout: LayoutStudent, components:{Head}, props:{ assignment:Object, submission:Object, page:Number, all_questions:Array, question_active:Object, question_answered:Number }, setup(props){
  const options=['A','B','C','D','E'];
  const goto=(p)=> router.get(`/student/assignments/${props.assignment.id}/${p}`);
  const submitAnswer=(question_id, ans)=>{ router.post(`/student/assignments/${props.assignment.id}/answer`,{ question_id, answer: ans },{ preserveState:true, preserveScroll:true }); };
