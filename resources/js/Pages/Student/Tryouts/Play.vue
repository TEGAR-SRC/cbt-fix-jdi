<template>
  <Head><title>Kerjakan Tryout - {{ tryout.title }}</title></Head>
  <div class="row mb-3"><div class="col-md-12"><h5 class="fw-bold mb-0">{{ tryout.title }}</h5><small class="text-muted">{{ tryout.lesson?.title }} • {{ tryout.classroom?.title }}</small></div></div>
  <div class="row">
    <div class="col-md-8">
    ...
    <div class="col-md-4">
      <div class="card border-0 shadow">
        <div class="card-body">
          ...
          <div class="d-flex justify-content-between"><span>Benar</span><strong>{{ attempt.total_correct }}</strong></div>
          <div class="d-flex justify-content-between"><span>Skor</span><strong>{{ attempt.score }}</strong></div>
          <hr>
          <Link :href="`/student/tryouts/${tryout.id}/finish`" method="post" as="button" class="btn btn-success w-100" confirm="Selesai?">Selesai</Link>
        </div>
      </div>
    </div>
  </div>
...
export default { layout: LayoutStudent, components:{Head,Link}, props:{ tryout:Object, attempt:Object, questions:Array }, setup(props){ const chosen = {}; const picked=(qid)=> chosen[qid]; const choose=(qid, ans)=>{ chosen[qid]=ans; router.post(`/student/tryouts/${props.tryout.id}/answer`, { question_id: qid, answer: ans }, { preserveScroll:true, preserveState:true }); }; return { picked, choose }; } }
</script>
<style></style>
