                ...
                <div class="card-body">
                    ...
                    <div v-if="question_active !== null">
                        ...
                        <div>
                            ...
                            <div v-if="question_active.question.image_path" class="mb-3">
                                <img :src="toStorageUrl(question_active.question.image_path)" alt="Gambar Soal" style="max-width: 100%; max-height: 300px;" />
                            </div>
                            <div v-if="question_active.question.audio_path" class="mb-3">
                                <audio :src="toStorageUrl(question_active.question.audio_path)" controls></audio>
                            </div>
                            <div v-if="question_active.question.video_path" class="mb-3">
                                <video :src="toStorageUrl(question_active.question.video_path)" controls style="max-width: 100%; max-height: 320px;"></video>
                            </div>
                        </div>
...
<script>
    ...
    export default {
        ...
        setup(props) {
            ...
            // to dedupe extra-time toasts
            const lastExtraToastAt = ref(0);

            // guard for leaving exam when on first question via browser back
            const exitGuardEnabled = ref(true);
                ...
                if (duration.value > 0) {
                    ...
                    if (counter.value % 10 == 1) {
                        ...
                        axios.put(`/student/exam-duration/update/${props.duration.id}`, {
                            ...
                            if (res?.data?.duration && res.data.duration !== undefined) {
                                duration.value = res.data.duration;
                            }
                            if (res?.data?.extra_ms && res.data.extra_ms >= 60000) {
                                const mins = Math.floor(res.data.extra_ms/60000);
                                const now = Date.now();
                                if (now - lastExtraToastAt.value > 5000) {
                                    lastExtraToastAt.value = now;
                                    Swal.fire({ title: 'Waktu Ditambah', text: `Anda mendapatkan tambahan waktu ${mins} menit.`, icon: 'info', timer: 2000, showConfirmButton: false });
                                }
                            }
                        });
                        // heartbeat for monitoring + sync
                        axios.post('/student/heartbeat', {
                            exam_id: props.exam_group.exam.id,
                            exam_session_id: props.exam_group.exam_session.id,
                            current_question: props.page,
                            duration: duration.value
                        }).then((res) => {
                            if (res?.data?.duration && res.data.duration !== undefined) {
                                duration.value = res.data.duration;
            ...
            // separate interval to send heartbeat every 15s as fallback
            ...
            onMounted(() => {
                ...
                // push a state so we can intercept back when on the first question
                try { history.pushState(null, '', location.href); } catch (e) {}
                const handlePop = (e) => {
                    if (!exitGuardEnabled.value) return;
                    if (props.page <= 1) {
                        // re-push state to cancel immediate back
                        try { history.pushState(null, '', location.href); } catch (err) {}
                        Swal.fire({
                            title: 'Keluar Ujian?',
                            text: 'Anda yakin ingin keluar dari sesi ujian?',
                            icon: 'warning',
                            ...
                            if (res.isConfirmed) {
                                axios.post('/student/exam-exit', {
                                    exam_group_id: props.exam_group.id,
                                    exam_id: props.exam_group.exam.id,
                                    exam_session_id: props.exam_group.exam_session.id,
                                }).then(() => {
                                    exitGuardEnabled.value = false;
                ...
                // store for removal
                window.__examHandlePop = handlePop;
            });

            onUnmounted(() => {
                if (heartbeatTimer) clearInterval(heartbeatTimer);
                if (window.__examHandlePop) {
                    window.removeEventListener('popstate', window.__examHandlePop);
                    delete window.__examHandlePop;
                }
            });
            ...
            return {
                ...
                endExam,
                toStorageUrl: (p) => p ? `/storage/${String(p).replace(/\\\\/g,'/').replace(/\\/g,'/')}` : ''
            }
