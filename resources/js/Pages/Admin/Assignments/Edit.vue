...
export default {
  ...
  props: { assignment: Object, lessons: Array, classrooms: Array },
  setup(props){
    const toInput = (dt) => dt ? new Date(dt).toISOString().slice(0,16) : '';
    const form = useForm({
      title: props.assignment.title,
      description: props.assignment.description,
      lesson_id: props.assignment.lesson_id,
      classroom_id: props.assignment.classroom_id,
      due_at: toInput(props.assignment.due_at),
      published_at: toInput(props.assignment.published_at)
    });
    const submit = () => form.put(`/admin/assignments/${props.assignment.id}`);
    return { form, submit };
  }
}
</script>
