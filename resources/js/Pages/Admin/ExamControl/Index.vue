<template>
  <Head>
    <title>Kontrol Ujian - Aplikasi Ujian Online</title>
  </Head>
          ...
          <div class="card-body">
            <div class="row g-3 align-items-end">
              ...
              <div class="col-md-6 text-end">
                <div class="text-muted">Gunakan halaman ini untuk Buka/Stop/Reopen percobaan ujian tanpa tampilan monitor.</div>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  const extraMinutes = ref({});
    ...
    const postWithToast = (url, successMsg) => {
      router.post(url, {}, {
        ...
        onSuccess: () => {
          ...
          // reload grades so status/time reflect immediately
    ...
    const confirmStop = (id) => {
      ...
      }).then((r) => { if (r.isConfirmed) postWithToast(`/admin/monitor/${id}/stop`, 'Ujian dihentikan.'); });
    };
    ...
    const confirmUnlock = (id) => {
      Swal.fire({
        ...
        cancelButtonText: 'Batal'
      }).then((r) => { if (r.isConfirmed) postWithToast(`/admin/monitor/${id}/unlock`, 'Ujian dibuka.'); });
    };
    ...
    const confirmReopen = (id) => {
      Swal.fire({
        ...
        cancelButtonText: 'Batal'
      }).then((r) => { if (r.isConfirmed) postWithToast(`/admin/monitor/${id}/reopen`, 'Ujian selesai dibuka kembali.'); });
    };
    ...
    const confirmAddTime = (id) => {
      ...
      Swal.fire({
        ...
        if (r.isConfirmed) {
          router.post(`/admin/monitor/${id}/add-time`, { extra_minutes: minutes }, {
            preserveScroll: true,
            onSuccess: () => {
              Swal.fire({ title: 'Berhasil', text: `Waktu +${minutes} menit.`, icon: 'success', timer: 1800, showConfirmButton: false });
              router.reload({ only: ['grades'] });
            }
          });
        }
    ...
    return { selectedSession, applyFilter, formatMs, confirmStop, confirmUnlock, confirmReopen, extraMinutes, confirmAddTime };
  }
}
</script>
