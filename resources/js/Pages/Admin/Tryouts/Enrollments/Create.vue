<template>
  <Head><title>Tambah Enrolle Tryout - {{ tryout.title }}</title></Head>
  <div class="container-fluid mb-5 mt-5">
    <Link :href="`/admin/tryouts/${tryout.id}/enrollments`" class="btn btn-md btn-primary border-0 shadow mb-3"><i class="fa fa-long-arrow-alt-left me-2"/> Kembali</Link>
...
export default { layout: AdminLayout, components:{Head,Link}, props:{ tryout:Object, students:Array, errors:Object }, setup(props){ const form=reactive({ student_id:[] }); const all=ref(false); const toggleAll=()=>{ form.student_id = all.value ? props.students.map(s=>s.id):[]; }; const reset=()=>{ form.student_id=[]; all.value=false; }; const submit=()=>{ router.post(`/admin/tryouts/${props.tryout.id}/enrollments`, form); }; return { form, all, toggleAll, reset, submit }; } }
</script>
<style></style>
