<template>
  <Head>
    <title>AI Import Soal - Aplikasi Ujian Online</title>
  </Head>
          ...
          <div class="card-body">
            ...
            <form v-if="!items.length" @submit.prevent="generate">
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <input v-model="form.category" type="text" class="form-control" placeholder="mis. Aljabar kelas 8, Teks Prosedur Bahasa Indonesia" required />
                </div>
                <div class="col-md-3">
              ...
              <div class="mt-4">
                <button class="btn btn-md btn-primary border-0 shadow" :disabled="loading || $page.props.jetstream?.inertia?.processing" type="submit">
            ...
            <div v-else>
              <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                  <strong>Editor Soal (AI/Manual)</strong> — Total: {{ items.length }}
                </div>
                <div class="btn-group">
...
export default {
  ...
  props: { exam: Object, errors: Object, preview: Array, form: Object, error: String },
  setup(props) {
    const form = reactive({ category: props.form?.category || '', count: props.form?.count || 5, difficulty: props.form?.difficulty || '' });
    const loading = ref(false);

    const items = ref((props.preview || []).map(n => ({ include: true, ...n })));
    const errorMsg = ref(props.error || (typeof window !== 'undefined' && window.__inertiaError) || '');

    // Sync when navigating back with inertia
    watch(() => props.preview, (val) => {
      items.value = (val || []).map(n => ({ include: true, ...n }));
      page.value = 1;
    });
    watch(() => props.error, (val) => { errorMsg.value = val || ''; });
    const page = ref(1);
    const pageSize = 5;
    ...
    const generate = () => {
      loading.value = true;
      router.post(`/admin/exams/${props.exam.id}/questions/ai-import/generate`, form, {
        onSuccess: (page) => {
          loading.value = false;
          const err = page?.props?.error || page?.props?.session?.error;
          if (err) {
            errorMsg.value = err;
            Swal.fire({ title: 'Gagal', text: err, icon: 'error', timer: 2500, showConfirmButton: false });
          } else {
            Swal.fire({ title: 'Berhasil', text: 'Soal berhasil digenerate. Silakan edit sebelum simpan.', icon: 'success', timer: 1800, showConfirmButton: false });
          }
        },
        onError: (errs) => {
          loading.value = false;
          errorMsg.value = 'Terjadi kesalahan saat generate.';
          Swal.fire({ title: 'Gagal', text: 'Terjadi kesalahan saat generate.', icon: 'error' });
        },
    ...
    const confirmImport = () => {
      const cleaned = items.value.map(it => ({
        ...
        options: Array.isArray(it.options) ? it.options.map(o => (o || '').trim()).slice(0,5) : [],
        answer: Number(it.answer) || 1,
      }));
      saving.value = true;
      router.post(`/admin/exams/${props.exam.id}/questions/ai-import/confirm`, { items: cleaned }, {
        preserveScroll: true,
        onSuccess: () => {
          saving.value = false;
          Swal.fire({ title: 'Berhasil', text: 'Soal disimpan ke bank soal.', icon: 'success', timer: 2000, showConfirmButton: false });
        },
        onError: () => {
          saving.value = false;
          Swal.fire({ title: 'Gagal', text: 'Gagal menyimpan. Cek isian Anda.', icon: 'error' });
        },
    ...
    const addItem = () => {
      items.value.push({ include: true, question: '', options: ['', '', '', '', ''], answer: 1 });
      page.value = totalPages.value; // jump to last
    };
    const removeItem = (i) => {
